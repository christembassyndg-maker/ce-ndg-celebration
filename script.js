const lines=[...document.querySelectorAll('.curtain-line')];
let lineIndex=0;
...
function fadeMusic(){
  ...
  let v=0;
  const f=setInterval(()=>{
    if(!musicOn){clearInterval(f); return;}
    v+=.025;
    audio.volume=Math.min(v,.55);
    if(audio.volume>=.55) clearInterval(f);
  },120);
}
document.getElementById('musicToggle').addEventListener('click',()=>{
  musicOn=!musicOn;
  document.getElementById('musicToggle').textContent=musicOn?'Music: On':'Music: Off';
  if(musicOn){audio.volume=.55;audio.play().catch(()=>{});}
  else audio.pause();
});

document.getElementById('openSeal').addEventListener('click',()=>{
  fadeMusic();
  document.getElementById('envelope').classList.add('opened');
  setTimeout(()=>{
    document.getElementById('envelopeScene').style.display='none';
    const panel=document.getElementById('mainInvite');
...
function updateCountdown(){
  const eventDate=new Date('2026-07-31T19:00:00-04:00').getTime();
  let diff=Math.max(0,eventDate-Date.now());
